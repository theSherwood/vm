...
| R11 | **Equivalence now fuzzed (Phase-1 scope).** The §7/§12.6 property — *inert in `NORMAL`* (instrumented == un-instrumented) and *round-trip* (freeze→serialize→restore→thaw ≡ uninterrupted, reload-not-reissue) — runs over a generator of **in-scope** durable modules: stable smoke `crates/svm-durable/tests/durable_fuzz.rs` (CI, no nightly) + libFuzzer target `fuzz/fuzz_targets/durable.rs`. Coverage broadens automatically as the transform generalizes (R8). Cross-backend (JIT) arm still to add. | §7, §12.6 | addressed (Phase-1 scope) |
