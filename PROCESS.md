...
| S7 | BusyBox port; stage-1/2 demo gates (**endpoint-free**) | S3,S4 | **Stage-0 mini-shell done** — a real read-eval loop (chibicc → `svm-posix`, libc by name) runs a script from preloaded stdin with builtins (`echo` incl. `$VAR`, `pwd`, `cd`, `exit`, unknown→`not found`), differential interp/JIT (`c_shell.rs`). Proves the command-interpreter pipeline end to end on the personality; surfaces the BusyBox `ash` gaps: **argv delivery** to a personality program, `stat`/`fstat`/`opendir`/`readdir` (command/path lookup, file tests), more `<stdio.h>`/`<string.h>` guest libc, then `fork`/`exec` (Stage 1+, on `Instantiator`/clone). BusyBox `ash` port itself: todo |
