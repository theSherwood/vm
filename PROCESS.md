...
| S3 | Lifecycle: `poll`/`kill`/`detach` (+ per-child kill cell on JIT) | S1 | **poll + detach + kill done** (interp; `Instantiator` ops 9/10/12): `poll` non-destructive 0/1/2 probe, `detach` drops the join claim, `kill` (op 12) sets a per-§14-child `Arc<AtomicBool>` polled at the per-op `step` — the child + its inherited-flag `thread.spawn` descendants trap (`ThreadFault` → `poll` 2). `lifecycle_kill.rs` (negative-checked). Reliably stops a **running** child; prompt-wake of a **parked** child + deep-`§14` subtree + JIT parity are follow-ups |
