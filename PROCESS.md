...
Open for consumer input (jacl) before building: whether guest-side self-mint
should also cover *detached* peers (it composes with the §5 detached-windows
trust design, where the data plane is a granted `SharedRegion`), and whether
`Endpoint` remains the right *name* for what is now "a live offer + the
domain's service points" (the mechanism §14 promised, delivered by §3.6).
...
| S9 | `Endpoint` v1 — **rescoped 2026-07-23** (see §4 "S9 rescope after §3.6"): serve/park/reply/cancel/FIFO/data-plane all landed via §3.6 (IMPORTS.md as-built); what remains is guest-side **self-mint** of a live offer (+ the dropped-serve/reply recommendation, pending consumer sign-off) | §3.6, S1b | rescoped — small residue, consumer-gated |
