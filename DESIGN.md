...
- **Trap-confinement (guard-when-bounded, bounds-check-when-not)** (D36). An
  ...
  - *Spectre-v1: the guard-select IS the hardening.* `select_spectre_guard` is a
    ...
    **0.94×/1.01×** (parity/faster). (A `trapnz`-keeping cmov variant — clamp only,
    branch kept — *regressed* to 1.23×/1.55×, confirming the branch removal is the
    win, not the cmov.) Nested §14 sub-windows keep the D38 `trapnz` + AND (no guard
    page at the child boundary). A per-domain unhardened mode (no spectre guard) is
    deliberately NOT offered: one fuzzed configuration, no deployment has justified
...
| D63 | **Branchless per-access confinement (`select_spectre_guard`), superseding D38's `trapnz` + AND clamp for top-level windows.** A non-elided top-level access lowers to `cmp; select_spectre_guard(oob, reserved, addr+offset); load` — an out-of-bounds address is redirected (branchlessly, via cmov) to `reserved`, the first byte of the trailing guard page, so the access itself faults there (`MemoryFault`). Same architectural fault and same clear-fault-at-the-offending-access property as D38; same Spectre-v1 confinement (`select_spectre_guard` is a speculation barrier — a misspeculated OOB access also lands on the guard), via the **identical Cranelift primitive Wasmtime uses**. Nested §14 sub-windows (`sub_base != 0`) keep D38's explicit `trapnz` + AND (no guard page at the child boundary, so a redirect could alias valid parent memory). | Settled (revises D38 for top-level; D38 retained for nested) | The A/B (`confine` harness) refuted D38's "1-op AND beats Wasmtime's cmp→cmov" claim: the 40-bit reservation mask is not an x86 immediate, so the AND was a **RIP-relative load per access**, and the `trapnz` added a per-access **branch** — together ~1.06×/1.34× behind Wasmtime-w64 on `matmul`/`matmul_eb`. Branchless cmov-to-guard hoists `reserved` into a register and removes the branch, reaching **0.94×/1.01×** (parity/faster). A `trapnz`-keeping cmov variant (clamp only) regressed (1.23×/1.55×) — the branch removal, not the cmov, is the win. Verification: OOB→`MemoryFault` differential + `fuzz/mask` (the OOB predicate is unchanged) + escape oracle (§18) |
