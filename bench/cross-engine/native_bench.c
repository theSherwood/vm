...
int64_t chase(int32_t), chase_rand(int32_t);
int32_t fnv(int32_t), fmar(int32_t), vsum(int32_t);
...
static double min_run64(int64_t (*k)(int32_t), int32_t n){
  sink+=(int32_t)k(n);
  double best=1e18;
  for(int r=0;r<25;r++){ double a=now(); sink+=(int32_t)k(n); double b=now(); if(b-a<best)best=b-a; }
  return best;
}
static void bench64(const char*name, int64_t (*k)(int32_t)){
  double s=min_run64(k,1000), l=min_run64(k,201000);
  printf("native,%s,%.4f\n", name, (l-s)/200000.0);
}
...
int main(){
  ...
  bench64("chase",chase); bench64("chase_rand",chase_rand);
  bench("fnv",fnv); bench("fma",fmar); bench("vsum",vsum);
  return 0;
}
