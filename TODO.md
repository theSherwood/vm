...
| **Serving-review items** — handler-trap blast radius (supervision/sharding idioms to document; poll→detach, never join a trapped child), servicer-side admission fairness, serialized-handler ceiling (document next to F6), `svc_results` orphan sweep, graceful revocation (tombstoned handles → errno, not trap) | ISSUES.md **I37 / I38 / I39 / I40 / I41** | recorded 2026-07-23/24; I38 + I40 + I41 are small substrate slices, I37 + I39 are documentation/pattern work |
| **Multi-consumer `svc.wait`** — N spawned server vCPUs drain one domain queue (the I39 opt-in ladder's top rung: parallel dispatch for domains that accept the threading discipline) | ISSUES.md I39 resolution path | queued **behind I36** (the native serve loops make it near-free; semantics pinned in the differential first) |
