...
/// An integer caller with a **float leaf** (integer signature — takes/returns i64 bits — but uses
/// f64 internally): the caller is in-subset, the leaf is interp-callable, the guest is mixed_ok.
/// This is the motivating mixed-tier shape.
#[test]
fn integer_caller_float_leaf() {
    let a = analyze(&m(r#"
func (i64) -> (i64) {
block0(v0: i64):
  v1 = call 1 (v0)
...
block0(v0: i64):
  ...
  return v3
    ...
    assert!(
        a.mixed_ok,
        "integer caller + memory-free float leaf must run mixed-tier"
    );
}

/// Func 0 itself is float → not in-subset → the JIT can't take the entry → not mixed_ok (the whole
/// guest stays on the interpreter).
#[test]
fn float_entry_not_mixed() {
    let a = analyze(&m(r#"
func (i64) -> (i64) {
block0(v0: i64):
  v1 = f64.convert_i64_s v0
...
/// A float callee that TOUCHES MEMORY is not an interp leaf (its fresh window would diverge from the
...
/// A float callee that itself CALLS another function is not a (true) leaf — transitive tiers are a
/// later refinement — so it blocks mixed_ok in this slice.
#[test]
fn non_leaf_float_blocks_mixed() {
    let a = analyze(&m(r#"
func (i64) -> (i64) {
block0(v0: i64):
  v1 = call 1 (v0)
...
block0(v0: i64):
  return v0
    ...
    assert!(
        !a.interp_leaf[1],
        "a non-leaf float function is not interp-callable yet"
    );
    assert!(!a.mixed_ok);
}
