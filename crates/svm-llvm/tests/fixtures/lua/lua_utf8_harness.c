/* Runs the unmodified official Lua 5.4.7 `testes/utf8.lua` (embedded as bytes in `lua_utf8_tests.c`)
 * with the base/string/table/math/utf8 libraries open. Same shape as `lua_testsuite_harness.c`, plus a
 * minimal `require`: utf8.lua opens with `local utf8 = require'utf8'`, so the harness installs a global
 * `require` that resolves a name from the registry `_LOADED` table (where `luaL_requiref` records every
 * opened library). That is exactly stock `require`'s fast path for an already-loaded module — the only
 * path reachable on the on-ramp, which has no filesystem or dynamic loader for the file/C searchers. */
#include "lua.h"
#include "lauxlib.h"
#include "lualib.h"
 ...
 * not reclaimed; sized for the file and reset by tearing down the `lua_State`. (See the test-suite
