...
//! engine dispatch through the in-process `Host`; the JIT calls out through `svm_run::cap_thunk` (the
//! real §9 trampoline), so the JIT number includes genuine marshalling + indirect-call dispatch.
//!
...
/// Time a module that takes `(clk, n)` (cap version) or `(n)` (baseline) on all three engines.
/// `has_cap` selects the arg shape and whether a clock is granted.
fn time_engines(m: &Module, has_cap: bool) -> (f64, f64, f64) {
    ...
    let jit = per_iter(|n| {
        // The JIT reaches the host through the real §9 trampoline; `ctx` is the live `Host`.
        let mut host = Host::new();
        let clk = host.grant_clock();
...
fn main() {
    ...
    let (tw_c, bc_c, jit_c) = time_engines(&cap, true);
    let (tw_b, bc_b, jit_b) = time_engines(&base, false);
    ...
    for (name, c, b) in [
        ...
        let cost = c - b;
        println!(
            "{name:<12} {c:>12.2} {b:>12.2} {cost:>14.2} {:>13.1}x",
            c / b.max(0.001)
        );
    }
    println!(
        "\n(cap.call cost = cap-loop per-iter − a no-cap loop of the same shape, so it isolates the\n \
         host-boundary crossing. The JIT routes through the real `svm_run::cap_thunk` (marshalling +\n \
         indirect dispatch); the interpreters dispatch the clock cap through the in-process Host. The\n \
         clock read is the cheapest cap — heavier caps (I/O, spawn) add their own work on top.)"
    );
}
