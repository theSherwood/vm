...
| 11c | **data dir + the rest of the runtime** | — | `initdb` natively → grant the dir through the (now tree-walkable) `fs` cap; proc/time/signal deterministic stubs (`getpid`/`clock_gettime`/`sigaction`/…); the ~180 remaining pure-libc externs (stdio `FILE*`, locale, ctype tables, `strtod`/`snprintf`) byte-exact vs the native oracle; storage manager, WAL, single-process shmem, catalog bootstrap — the ~50 *live* externals resolve here |
...
native glibc oracle. Still ahead (gap #11c): the proc/time/signal stubs, the ~180 remaining pure-libc
externs (stdio `FILE*`, locale, ctype, `strtod`/`snprintf`) byte-exact vs native, and then the storage
manager, WAL, single-process shmem, and catalog bootstrap. See `LLVM.md` slices BM–CA.
