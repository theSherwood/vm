...
| 11d | **data dir + the rest of the runtime** | — | `initdb` natively → grant the dir through the (now tree-walkable) `fs` cap; proc/time/signal deterministic stubs (`getpid`/`clock_gettime`/`sigaction`/…); the remaining pure-libc externs (stdio `FILE*`, locale, `strtod`/`snprintf`) byte-exact vs the native oracle; storage manager, WAL, single-process shmem, catalog bootstrap — the ~50 *live* externals resolve here |
...
native glibc oracle — and `libc_shim.c` (gap #11c) starts the pure-libc surface with the C-locale
**ctype** tables the scanner needs. Still ahead (gap #11d): the proc/time/signal stubs, the remaining
pure-libc externs (stdio `FILE*`, locale, `strtod`/`snprintf`) byte-exact vs native, and then the
storage manager, WAL, single-process shmem, and catalog bootstrap. See `LLVM.md` slices BM–CB.
