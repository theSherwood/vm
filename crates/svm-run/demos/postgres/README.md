...
resolves (via `svm_interp::func_name`) to the extern, the stub body staying a pure `Unreachable`. That
...
`OPENLIBM_SRCS` — the 18 transcendentals `log`/`exp`/`pow`/`sin`/… the SVM has no op for; `sqrt`/`fabs`/
