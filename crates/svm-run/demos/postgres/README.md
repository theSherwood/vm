...
| 11f | **data dir + the rest of the runtime** | — | `initdb` natively → grant the dir through the (now tree-walkable) `fs` cap; the stream `FILE*` (`stdout`/`stderr`/`stdin` → powerbox Stream, needs an on-ramp fd-dispatch) + the varargs `fprintf`/`scanf` format engines; storage manager, WAL, single-process shmem, catalog bootstrap — the ~50 *live* externals resolve here |
...
**process/time/signal** stubs, the file-backed **stdio `FILE*`**, and **time + wide-char**. Still ahead
(gap #11f): the stream `FILE*` (`stdout`/`stderr` → the powerbox Stream, needing an on-ramp fd-dispatch)
and the varargs `fprintf`/`scanf` format engines, then the storage manager, WAL, single-process shmem,
and catalog bootstrap. See `LLVM.md` slices BM–CD.
